...
def generalseek():
    ...
    total_pv=get_profile_visits(user_id)
    
    return render_template('seeker/dashboard.html',total_apps=total_apps,total_pv=total_pv)
...
def get_profile_visits(user_id):
    cursor = model3.get_cursor()
    query = "SELECT COUNT(*) FROM profile_visit_log WHERE seeker_profile_id = %s"
    cursor.execute(query, (user_id,))
    total_pv = cursor.fetchone()[0]  # Fetch the count from the first column of the first row
    cursor.close()  # Close cursor
    return total_pv
...
def getprofile(user_id):
    ...
    query = """
        SELECT 
            ...
            u.user_image
        FROM 
            db3.job_seeker_profile jp
        INNER JOIN
            db3.education_detail c ON jp.user_account_id = c.user_account_id
        INNER JOIN 
            db3.experience_detail l ON jp.user_account_id = l.user_account_id
        INNER JOIN 
            db3.skill_set s ON jp.user_account_id = s.user_account_id
        INNER JOIN 
            db3.seeker_skill_set f ON s.skill_set_id = f.id
         INNER JOIN
            db1.user_account u ON jp.user_account_id = u.id
