...
def get_images():
    cursor = cnx.cursor()
    cursor.execute("SELECT image_url FROM db2.company_image")
...
def hello_flask():
    ...
    return render_template('auth/index4.html',images=images)
...
def services():
    return render_template('auth/sevices.html')
...
def update_post(post_id):
    ...
    # For example, you could raise an error or set a default value
            ...
            if selected_skills:
                ...
                query = "DELETE FROM job_post_skillset WHERE job_post_id = %s"
                cursor4.execute(query, (post_id,))
        # Commit the transaction
                model4.commit()
